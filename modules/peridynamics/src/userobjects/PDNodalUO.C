...
InputParameters validParams<PDNodalUO>()
{
  ...
  params.addCoupledVar("nodal_volsum", "Auxiliary variable for sum of neighbor volume at each node");
...
PDNodalUO::PDNodalUO(const InputParameters & parameters) :
  ...
  _nodal_volsum_var(getVar("nodal_volsum", 0)),
...
void
PDNodalUO::initialize()
{
  std::vector<std::string> zero_vars;
  zero_vars.push_back("nodal_volsum");
...
PDNodalUO::execute()
{
  ...
  long int nv_dof0 = _current_elem->get_node(0)->dof_number(_aux.number(), _nodal_volsum_var->number(), 0);
  long int nv_dof1 = _current_elem->get_node(1)->dof_number(_aux.number(), _nodal_volsum_var->number(), 0);
  ...
  {
    _aux.solution().add(nv_dof0, _pdmesh.volume(node1));
    _aux.solution().add(nv_dof1, _pdmesh.volume(node0));
    _aux.solution().add(nd_dof0, val * _pdmesh.volume(node1));
    _aux.solution().add(nd_dof1, val * _pdmesh.volume(node0));
  }
  else
  {
    _aux.solution().add(nv_dof0, 0.0);
    _aux.solution().add(nv_dof1, 0.0);
...
void
PDNodalUO::threadJoin(const UserObject & uo)
{
}
...
Real
PDNodalUO::computeVolSum(unsigned int node_id) const
{
  NumericVector<Number> & sln = _aux.solution();
  long int nv_dof = _mesh.nodePtr(node_id)->dof_number(_aux.number(), _nodal_volsum_var->number(), 0);

  return sln(nv_dof);
}

Real
PDNodalUO::computeDilatation(unsigned int node_id) const
{
  NumericVector<Number> & sln = _aux.solution();
  long int nd_dof = _mesh.nodePtr(node_id)->dof_number(_aux.number(), _nodal_dila_var->number(), 0);
