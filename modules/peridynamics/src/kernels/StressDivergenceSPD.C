...
StressDivergenceSPD::StressDivergenceSPD(const InputParameters & parameters) :
  ...
  _bond_force_i(getMaterialProperty<Real>("bond_force_i")),
  _bond_force_j(getMaterialProperty<Real>("bond_force_j")),
  _bond_dfdU_ij(getMaterialProperty<Real>("bond_dfdU_ij")),
  _bond_dfdU_i(getMaterialProperty<Real>("bond_dfdU_i")),
  _bond_dfdU_j(getMaterialProperty<Real>("bond_dfdU_j")),
...
StressDivergenceSPD::computeResidual()
{
  ...
  NonlinearSystem & nsys = _fe_problem.getNonlinearSystem();
  const NumericVector<Number> & sol = *nsys.currentSolution();
  ...
  for (unsigned int k = 0; k < neighbors.size(); ++k)
  {
    ...
    for (unsigned int j = 0; j < _pddim; ++j)
    {
      ...
      ori_ik(j) = _pdmesh.coord(neighbors[k])(j) + sol(nd_k->dof_number(nsys.number(), j, 0)) - _pdmesh.coord(node_i)(j) - sol(nd_i->dof_number(nsys.number(), j, 0));
    }
    origin_length = std::sqrt(origin_length);
    ori_ik /= ori_ik.size();

    _local_re(0) = - _bond_force_i[0] * vol_k / origin_length * ori_ik(_component);
    _local_re(1) = - _local_re(0);
  ...
  for (unsigned int k = 0; k < neighbors.size(); ++k)
  {
    ...
    for (unsigned int j = 0; j < _pddim; ++j)
    {
      ...
      ori_jk(j) = _pdmesh.coord(neighbors[k])(j) + sol(nd_k->dof_number(nsys.number(), j, 0)) - _pdmesh.coord(node_j)(j) - sol(nd_j->dof_number(nsys.number(), j, 0));
    }
    origin_length = std::sqrt(origin_length);
    ori_jk /= ori_jk.size();

    _local_re(0) = _bond_force_j[0] * vol_k / origin_length * ori_jk(_component);
    _local_re(1) = - _local_re(0);
...
StressDivergenceSPD::computeFullJacobian()
{
  ...
  for (unsigned int k = 0; k < neighbors.size(); ++k)
  {
    ...
    double stiff_elem1 = ori_ik(_component) * ori_ij(_component) * _bond_dfdU_i[0];
    double stiff_elem2 =  _bond_force_i[0] * (1.0 - ori_ik(_component) * ori_ik(_component)) / current_length;
  ...
  for (unsigned int k = 0; k < neighbors.size(); ++k)
  {
    ...
    double stiff_elem1 = - ori_jk(_component) * ori_ij(_component) * _bond_dfdU_j[0];
    double stiff_elem2 = _bond_force_j[0] * (1.0 - ori_jk(_component) * ori_jk(_component)) / current_length;
  ...
  else
  {
    ...
    {   
      ...
      for (unsigned int k = 0; k < neighbors.size(); ++k)
      {
        ...
        {
          off_stiff_elem1 = ori_ik(_component) * ori_ij(coupled_component) * _bond_dfdU_i[0];
          off_stiff_elem2 = - _bond_force_i[0] * ori_ik(_component) * ori_ik(coupled_component) / current_length;
        }
      ...
      for (unsigned int k = 0; k < neighbors.size(); ++k)
      {
        ...
        {
          off_stiff_elem1 = - ori_jk(_component) * ori_ij(coupled_component) * _bond_dfdU_j[0];
          off_stiff_elem2 = - _bond_force_j[0] * ori_jk(_component) * ori_jk(coupled_component) / current_length;
        }
