      ...
      const GenericMaterialProperty<RankTwoTensor, is_ad> & strain_rate)
{
  ...
  Real integral = sigma_eq * strain_rate_eq;

  // Perform the integral using Gaussian quadrature
  for (unsigned int k = 0; k < 5; ++k)
  {
    // Transform Gaussian points to the interval [0, sigma]
    GenericReal <is_ad> sigma_eq_tmp;
    sigma_eq_tmp = 0.5 * (points[k] + 1) * sigma_eq; // Map to [0, sigma_eq]
    Real strain_rate_tmp = MetaPhysicL::raw_value(this->computeResidual(sigma_eq_tmp, 0.0)) / _dt;
    integral -= 0.5 * weights[k] * sigma_eq * strain_rate_tmp;
  }
  return integral;
}
